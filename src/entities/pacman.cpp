...
void Pacman::update(const sf::Time& time){
	if(!dying){
		...
		if(map.getField(pos2).isCanPass()){
			...
			sprite.setPosition(pos.x * GraphicSettings::fieldSize + dx[dir] * change,
			                   pos.y * GraphicSettings::fieldSize + dy[dir] * change);
			sprite.update(time);
		}
		else{
			if(dirKeyboard != None && dir != dirKeyboard){
		...
		sprite.update(time);
		deltaTime += time;
		if(deltaTime >= sf::milliseconds(1200)){
			active = true;
			sprite.setAnimation(AssetManager::get().pacman.left);
			...
			GameEvent event;
			event.type = GameEvent::PacmanStarted;
...
void Pacman::onNotify(const GameEvent& event){
	if(event.type == GameEvent::PacmanEaten){
		...
		dying = true;
		sprite.setAnimation(AssetManager::get().pacman.death);
	}
}
